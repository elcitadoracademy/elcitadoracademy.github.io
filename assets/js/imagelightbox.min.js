...
_pushToHistory();target=targets.eq(targetIndex);$wrapper.trigger('next.ilb2',target);_loadImage(-1)},activityIndicatorOn=function(){$wrapper.append($activityObject)},activityIndicatorOff=function(){$('.imagelightbox-loading').remove()},overlayOn=function(){$wrapper.append($overlayObject)},closecuttonOn=function(){$cuttonObject.appendTo($wrapper).on('click.ilb7',function(){_quitImageLightbox();return!1})},captionReset=function(){$captionObject.css('opacity','0');$captionObject.html('&nbsp;');if($(target).data('ilb2-caption')){$captionObject.css('opacity','1');$captionObject.html($(target).data('ilb2-caption'))}else if($(target).find('img').attr('alt')){$captionObject.css('opacity','1');$captionObject.html($(target).find('img').attr('alt'))}},navigationOn=function(){if(!targets.length){return}
